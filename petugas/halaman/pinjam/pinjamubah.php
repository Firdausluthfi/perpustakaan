<a href="index.php?page=pinjam"><button>Kembali ke Data Pinjam</button></a>
<br /><br />
<?php
    include "../koneksi.php";
    $id_pinjam = $_GET['id_pinjam'];
    $ambildata = mysqli_query($sambung, "SELECT * FROM tbl_pinjam INNER JOIN tbl_petugas ON tbl_petugas.idpetugas=tbl_pinjam.idpetugas INNER JOIN tbl_siswa ON tbl_siswa.idsiswa=tbl_pinjam.idsiswa INNER JOIN tbl_buku ON tbl_buku.idbuku=tbl_pinjam.idbuku WHERE id_pinjam='$id_pinjam'");
    ...
    <form action="halaman/pinjam/pinjamubah_aksi.php" method="post" name="formubah">
        <table>
            <tr>
                <td>ID Pinjam</td>
                <td>
                    <input type="text" name="id_pinjam" value="<?php echo $tampildata['id_pinjam'] ?>" readonly>
                </td>
            </tr>
